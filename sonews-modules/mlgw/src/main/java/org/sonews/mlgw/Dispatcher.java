...
public class Dispatcher {
    ...
    /**
     * This method inspects the header of the given message, trying to find the
    ...
    /**
     * Posts a message that was received from a mailing list to the appropriate
     * newsgroup. If the message already exists in the storage, this message
     * checks if it must be posted in an additional group. This can happen for
    ...
    public static boolean toGroup(final Message msg) {
        ...
        try {
            ...
            Article article = StorageManager.createArticle(msg);
            ...
            if (updateReq) {
                ...
                if (oldArticle != null) {
                    ...
                    oldGroups.stream().filter((oldGroup) -> (!newsgroups.contains(oldGroup.getName()))).forEach((oldGroup) -> {
            ...
            if (newsgroups.size() > 0) {
                ...
                if (updateReq) {
                    Log.get().log(Level.INFO, "Updating {0} with additional groups", article.getMessageID());
                    StorageManager.current().delete(article.getMessageID());
                    StorageManager.current().addArticle(article);
                } else {
                    Log.get().log(Level.INFO, "Gatewaying {0} to {1}", new Object[]{article.getMessageID(), article.getHeader(Headers.NEWSGROUPS)[0]});
                    StorageManager.current().addArticle(article);
                }
                posted = true;
            } else {
                ...
                Log.get().log(Level.WARNING, "No group for{0}", buf.toString());
            }
            return posted;
     ...
     * This method MAY be called several times by Post
     * @param article
     * @param group
     * @return 
    ...
    public static boolean toList(Article article, String group)
        ...
        for (String rcptAddress : rcptAddresses) {
            ...
            Log.get().log(Level.INFO, "MLGateway: Mail {0} was delivered to {1}.", new Object[]{article.getHeader("Subject")[0], rcptAddress});
        }
        return true;
    }
