...
/**
 * Filter input stream for reading from SMTP (or NNTP or similar) socket where
 * lines containing single dot have special meaning – end of message.
 * 
...
public class SMTPInputStream extends FilterInputStream {
    ...
    /**
     * @return one byte as expected or -2 if there was line with single dot
    ...
    public int read() throws IOException {
        ...
        if (ch == DOT) {
            if (last == LF) {
                ...
                if (next == CR || next == LF) { // There should be CRLF, but we
                                                // may accept also just LF or CR
                                                // with missing LF. Or should we
                                                // be more strict?
     ...
     * @param offset
     * @param length
     * @return See {@link FilterInputStream#read(byte[], int, int)} or -2 (then
     *         see {@link #read(byte[])})
     * @throws IOException
     */
    @Override
