...
/**
 * The XDAEMON command allows a client to get/set properties of the running
 * server daemon. Only locally connected clients are allowed to use this
 * command. The restriction to localhost connection can be suppressed by
 * overriding the sonews.xdaemon.host bootstrap config property.
 * 
...
public class XDaemonCommand implements Command {
    ...
    private void channelAdd(String[] commands, NNTPConnection conn)
        ...
        if (StorageManager.current().isGroupExisting(groupName)) {
            ...
            StorageManager.current().addGroup(groupName,
        ...
        if (addr.getHostName().equals(
            ...
            if (commands.length == 3 && commands[1].equalsIgnoreCase("LIST")) {
                if (commands[2].equalsIgnoreCase("CONFIGKEYS")) {
                    ...
                    List<Subscription> pull = StorageManager.current()
                            .getSubscriptions(FeedManager.TYPE_PULL);
                    List<Subscription> push = StorageManager.current()
                            .getSubscriptions(FeedManager.TYPE_PUSH);
                    conn.println("100 list of peering rules follows");
                    for (Subscription sub : pull) {
                        conn.println("PULL " + sub.getHost() + ":"
                                + sub.getPort() + " " + sub.getGroup());
                    }
                    for (Subscription sub : push) {
                        conn.println("PUSH " + sub.getHost() + ":"
                                + sub.getPort() + " " + sub.getGroup());
                    }
                    conn.println(".");
                } else {
                ...
                if (group == null) {
                    ...
                    group.update();
                    conn.println("200 group " + commands[2]
                            + " marked as deleted");
                }
            } else if (commands.length == 5
                    && commands[1].equalsIgnoreCase("GROUPFLAG")) {
                Group group = StorageManager.current().getGroup(commands[2]);
                ...
                if (val != null) {
                    ...
                    && commands[1].equalsIgnoreCase("LOG")) {
                Group group = null;
                ...
                if (commands[2].equalsIgnoreCase("CONNECTED_CLIENTS")) {
                    ...
                    conn.println(Integer.toString(Stats.getInstance()
                            .connectedClients()));
                    conn.println(".");
                    ...
                    for (int n = 0; n < 24; n++) {
                        conn.println(n
                                + " "
                                + Stats.getInstance().getYesterdaysEvents(
                                        Stats.POSTED_NEWS, n, group));
                    }
                    conn.println(".");
                    ...
                    conn.println(Double.toString(Stats.getInstance()
                            .postedPerHour(-1)));
                    conn.println(".");
                    ...
                    conn.println(".");
                        ...
                        .equalsIgnoreCase("GATEWAYED_NEWS_PER_HOUR")) {
                    ...
                    && commands[1].equalsIgnoreCase("PLUGIN")) {
